...
- 不要让 Session / Tool / Memory / Skill / Artifact / Workspace 等 owner module 拥有“用户可见/模型可见/Trace 可见”等观察可见性；owner module 只保存事实和 refs，观察面由 Context Workspace slice / renderer 决定。
...
- Prompt 上下文真相由 Context Workspace 持有：Context Tree 节点、节点状态、observation slice、render snapshot 和 provider attachment mirror 落在 `context_workspace`。orchestration run 只记录 `context_snapshot_id` / `llm_request_slice_id` 等引用事实。
