...
### Runtime Contract 是最高层 prompt 节点
...
`content_hash`，`ContextRenderSnapshot` 与后续 LLM invocation 都要能结构化追踪本次使用的
...
### Phase 4：RunPromptInput 收口

- 将当前 `available_tools` 文本、skills catalog、memory recall、workspace bootstrap、session segment 逐步迁到 Context Tree。
- `RunPromptInputBuilder` 只保留 Context Workspace render、LLM routing、provider call 所需薄逻辑，不再拥有最终 prompt 拼装真相。
- 删除长期双轨和大段兼容 shim。
