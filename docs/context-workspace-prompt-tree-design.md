# Context Workspace / Context Tree 设计

本文记录 Context Workspace 和 Context Tree 的目标设计。早期文档名仍带
`prompt-tree`，但当前施工口径已经收敛为：Context Workspace 维护 canonical
Context Tree / Context Snapshot；LLM provider adapter / renderer 根据 provider、transport
和 model 能力把 runtime request 渲染成 provider wire input。Context Tree 不再被视为
provider prompt 的同义词。

> 2026-06-16 当前收敛口径：Context Tree 是会话上下文控制面，拥有
> selection/control state，不拥有 owner 数据真相。完整目标树形和迁移 checklist 见
> [reports/context-tree-control-state-target-design-20260616.md](reports/context-tree-control-state-target-design-20260616.md)。

## 背景

重构前系统由 orchestration 内部的 prompt 拼装器运行时拼装 provider input，来源包括 agent instruction、runtime context、flow hint、session transcript、workspace bootstrap、available tools、session tools、skills catalog、memory recall 和 artifact materialization。当前主线已经转向 `context_workspace` 维护真实 Context Tree，orchestration 只收集 runtime request draft、Context Snapshot refs、tool surface refs 和路由所需的薄上下文。

这套实现能运行，但它仍然是“拼装器”：

- tool、skill、memory、session、workspace 的上下文装配逻辑散落在 orchestration prompt 代码里。
- tool schema 目前偏“可用就带”，缺少注意力治理。
- session segment、compaction summary、archived messages 是隐式窗口逻辑，agent 不能直观看见或控制。
- 历史图片和文件为了压缩会被替换为占位，agent 很难主动回看。
- 人类和 Operations 面很难解释“这次 LLM 到底看到了什么、为什么没看到什么”。

我们要把上下文交付从“拼一段 provider prompt”升级为：

**维护 agent 和本地 runtime 共治的 Context Tree。树是 runtime canonical context；provider-specific tools/images/files/wire input 由 LLM renderer 从 runtime request 和 Context Snapshot 派生。**

## 目标

1. **树化上下文**
   Runtime 维护的是一棵已授权的上下文树，而不是一坨临时拼接文本。Session、Memory、Skill、Tool、Workspace、Artifact 都是节点；模型实际看到的 provider input 由 LLM renderer 生成。
...
### Runtime Contract 是最高层 runtime context 节点

所有 normal turn 都必须先挂入 `runtime.contract` 节点。其内容来自运行时文件
`src/crxzipple/modules/context_workspace/application/prompts/runtime_contract.md`，
不是 agent profile 或 provider fallback。节点 metadata 写入 `contract_version` 和
`content_hash`，`ContextSnapshot` 与后续 LLM invocation 都要能结构化追踪本次使用的
contract version/hash。

优先级为：

1. `runtime.contract`
2. 当前 agent home 文件，例如 `AGENT.md`、`USER.md`、`SOUL.md`、`IDENTITY.md`
3. 可选任务 workspace resource，例如绑定工作目录中的 `AGENTS.md`、`BOOTSTRAP.md`、`TOOLS.md`
4. 当前用户输入、session、tool result、memory、skill、artifact 等节点

CRXZipple 不是纯编程 agent runtime，不能把 project instruction 作为通用二级总纲。
Agent home 文件不再作为 workspace resource 混入；workspace resource 只表示当前 session
显式绑定工作目录后的可选文件句柄。
...
### 树是 canonical context，不是 provider prompt

调用 LLM 时，Context Tree 先冻结为 Context Snapshot。LLM provider renderer 再根据
provider / transport / model 能力，把 runtime request、transcript、tool surface 和
Context Snapshot 渲染成 provider wire input。

```text
Context Tree / Context Snapshot
...
Provider Renderer
  ├─ provider-native input items / messages
  ...
  └─ render report / loss report / wire preview
```

Provider input 是从 runtime canonical request 派生的结果。完整树 body 只应作为 debug
render 或 `context_tree.*` 工具结果进入 transcript，不作为默认 provider prompt body。
...
### Tool 能力按 source-first 分组

LLM 不关心工具来自 local package、OpenAPI 还是 MCP 的实现细节，但稳定分类需要减少注意力分散。
因此 `tools.available` 下优先展示 Tool Source / Bundle，再展示 source 内的能力 group，只有 group
展开后才暴露 tool function 和可镜像 schema。显式 `prompt.groups` 优先；没有显式 group 时，
Tool owner module 自动生成 source-level group。CLI source 仍是命令行引导能力，不自动变成
provider function。
...
- `ContextSnapshot`：某次 run 调用 LLM 时的上下文树状态、debug render 和 provider attachment mirror 快照。
...
- Run 是短生命周期执行单元，只需要记录当时 runtime context snapshot；LLM 实际看见的 provider input 由 LLM invocation 的 wire preview / render report 说明。
- Agent 不应全局共享树状态，否则不同 session 会互相污染。

## 树节点模型

内部使用 typed JSON/domain model。XML-like render 只作为 debug body 或显式
`context_tree.*` 工具输出，不是默认 provider prompt body。
...
<context_tree session="session-key" revision="42">
  ...
  <node id="render" kind="render_root" state="expanded">
    <summary>Current slice, provider payload mapping, and budget reports.</summary>
  </node>
</context_tree>
```

Provider prompt/request body must be rendered from Context Slice and provider
renderer output, not from this debug tree body.
...
- `record_context_snapshot(run_id, snapshot)`
...
长期目标是 orchestration 不再直接拼 memory、skill、tool、workspace、session segment 文本。
...
orchestration run
  ↓
context_workspace.build_context_snapshot(...)
context_workspace.render_observation_slice(..., audience=llm_request)
...
- 当前上下文压力。
- 上一次 run 的 Context Snapshot 与 provider render report。
- 哪些节点被预算降级。
...
- `context.provider_render_report.created`
- `context.budget.applied`

每次 run 必须保存当时 Context Snapshot 和 provider render report，否则树后续变化后无法复现当时 LLM 看到的上下文。
...
- 定义 domain model：workspace、node、node state、operation、estimate、Context Snapshot、provider render report。
...
- 通过 Context Slice 选择 session/runtime refs；runtime replay window 和 provider input items 由 runtime request builder 与 LLM renderer 分层生成，不读取 tree debug body。
- 建立 run snapshot 记录。

### Phase 3：Tool / Skill / Memory / Artifact 节点

- Tool 提供 slice-visible tool nodes 和 schema attachment extraction。
- Skills 提供 ready skill nodes 和 read_skill action。
- Memory 提供 slice-visible scope nodes 和 recall_memory action。
- Artifacts 提供 image/file nodes、caption、thumbnail、payload handle。

### Phase 4：RuntimeLlmRequestDraft 收口

- 将当前 `available_tools` 文本、skills catalog、memory recall、workspace bootstrap、session segment 逐步迁到 Context Tree。
- `RuntimeLlmRequestDraftCollector` 只保留 Context Snapshot refs、LLM routing、provider call 所需薄逻辑，不再拥有最终 provider input 拼装真相。
- 删除长期双轨和大段兼容 shim。

### Phase 5：UI / Operations

- Settings/Workbench/Operations 增加 Context Tree 可视化和操作面。
- Operations read model 增加 context workspace health、node operations、run Context Snapshot / render report、budget estimate。
- UI 显示每个节点的 prompt/provider 成本，支持人类 pin/collapse/disable schema。

### Phase 6：清理旧模式

- 清理 orchestration 里直接拼 tool/skill/memory/workspace/session segment 的旧路径。
...
Context Tree 冻结为 Context Snapshot
        ↓
LLM Provider Renderer 生成 provider wire input / render report
```

一句话：

**树是 runtime canonical context，不是 provider prompt；provider 特化输入由 LLM renderer 从 runtime request 和 Context Snapshot 派生。**
