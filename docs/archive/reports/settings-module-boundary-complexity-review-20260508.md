...
### R11. Skill Catalog 已回到 Skills owner，Settings 只做治理入口

Skill enablement overlay 已退场；`SkillEnablementManagerAdapter` 和
`src/crxzipple/modules/skills/application/settings_integration.py` 不再是运行时路径。

风险：

- 如果后续在 Settings 页新增 skill 操作，必须 dispatch 到 Skills application/API，
  不能写 Settings resource 后再要求 runtime sync。
- UI 展示安装、manifest、readiness、source、运行能力时，必须标注 owner 为 Skills。
...
| skill-catalog | module-owned-entity | Skills | Settings 只做治理入口；创建、安装、manifest、启停、source、readiness、read/catalog 都走 Skills owner API |
...
### C3B. Skill Catalog 收口

状态：基本完成，剩余为 UI 细节和审计增强。

- [x] 删除 Settings-owned skill enablement overlay。
- [x] 删除 `SkillEnablementManagerAdapter` 运行时注入。
- [x] Skill 页面改读 `/skills/*` owner API。
- [x] create/update/write/read/install/validate/enable/disable/delete/sync/source
  等动作走 Skills application/API。
- [ ] 详情区继续补齐 Instructions、Resources、Readiness、Source、Audit 的高密度展示。
- [ ] 独立 installation/audit 记录仍归 Skills owner 后续补齐。

验收：

- Settings 不再自称拥有 skill package/catalog truth。
- Skills runtime usage 从 Orchestration resolver / skill_read 运行事实和
  Operations/Skills 观察面进入，不从 Settings overlay 反推。
