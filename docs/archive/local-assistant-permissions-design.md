...
> Archived: this early local-permissions design is historical. Current permission/access work follows the `access` and `authorization` modules plus `AGENTS.md`.
...
## Tool Preference Ownership

Agent profiles do not carry task-level tool preferences.

Task-level tool preference belongs in skill requirements:
...
These fields express skill applicability and guidance. They do not mean `allow`.
Final permission remains an ABAC decision.
...
- identity and instruction policy
- LLM routing policy
- execution policy
- runtime preferences such as home/workdir/sandbox defaults
...
## Example Skill Requirement Shape
