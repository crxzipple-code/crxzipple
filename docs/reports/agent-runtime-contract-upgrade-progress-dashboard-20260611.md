...
- Codex WebSocket 才使用 `previous_response_id + delta input`。
...
- `llm-request-canonical-input-alignment-plan` checklist 已按已验证实现校准：context slice report refs、request-side projector、provider renderer boundary、provider context -> Codex instructions、loss report、Operations/Trace inspection 与 Codex WebSocket continuation 持久化/回放闭环均已标为完成。
...
- Codex HTTP-only 仍能形成较强探索链，但会全量回放并触发 TPM 压力；WebSocket continuation 是效率底座，不是探索能力唯一来源。
- 当前优先级继续保持：
  - P0: Codex WebSocket transport/delta continuation 贯通并可观测。
  ...
  - P1: failure evidence ledger 让失败结论可审计、可续跑。
