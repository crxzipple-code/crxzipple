...
6. `/operations/*` HTTP read surface 从 `container.operations_projection_store` 读取物化投影；缺 projection 时返回可诊断错误，而不是在请求线程里大范围扫库/扫 topic。
...
- 轻量观察状态：`src/crxzipple/modules/operations/infrastructure/observation_store.py`
...
| Access | `access.py` | `collect_access_inventory(...)` 聚合 access、LLM、tool、channel services | access events from event bus + observation | `auth_success_rate` 实际是 readiness share，不是历史认证成功率；历史认证尝试缺专用计数源。 |
| Channels | `channels.py` | `channel_profile_service`、`channel_runtime_manager`、`channel_interaction_service` | 直接扫 channel/session/dead-letter event topics | 当前未接 `operations_observation_store`；趋势基于 recent event scan，不是长窗口聚合表。 |
...
| Skills | `skills.py` | `skill_manager`、`tool_service`、`access_service`、`agent_service` | skill/resolution events from event bus + observation | “top used” 当前更接近 requirement footprint，不是实际运行使用量；实际 usage 需要 run/tool/agent 事件聚合。 |
| Events | `events.py` | `events_service` topics/snapshots/cursors、event contract/definition registry | observer runtime subscriptions + event cursors | 能看 subscription lag，但缺 observer 进程心跳/实例状态；长周期事件量需要聚合存储。 |
...
1. 页面 projection 已经进入 Postgres `operations_projections`，但 `OperationsObservationStore` 的 observed events / heartbeat 仍是 file-backed；多 API/observer 或容器化后应继续迁到共享持久层。
2. 多个 provider 依赖 `Any` service 反射调用，应补正式 Operations query port/protocol，让数据契约稳定。
3. 事件类图表多为 recent topic scan，缺按时间窗口聚合的 durable read store。
...
6. Skills 的实际使用量、Access 的历史认证成功率，目前还没有真实事件聚合来源。

## 下一步落点

1. 把 observed events / observer heartbeat 从 file-backed observation store 继续迁到共享持久层，和 `operations_projections` 收敛。
...
5. 把 Access auth attempts、Skills runtime usage 明确成事件事实，再由 Operations observer 聚合。
