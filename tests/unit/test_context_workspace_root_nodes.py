...
def test_default_root_node_seeds_group_runtime_agent_execution_and_session_roots() -> None:
    ...
    assert by_id["run.goal"].parent_id == root_nodes.EXECUTION_CURRENT_NODE_ID
    ...
    assert by_id["evidence.frontier"].parent_id == root_nodes.EXECUTION_CURRENT_NODE_ID
...
def test_default_root_node_seeds_add_workspace_resource_root_when_bound() -> None:
    ...
    assert workspace_root.parent_id is None
...
def test_default_parent_id_for_known_non_root_nodes() -> None:
    ...
    assert root_nodes.default_parent_id_for_node_id("tools.available") is None
