...
_CLIENT_PROBE_EXPRESSION = """
/*__crxzipple_browser_client_probe__*/
(raw) => {
  const input = raw && typeof raw === "object" ? raw : {};
  const objectPath = String(input.object_path || input.objectPath || "").trim();
  const methodName = String(input.method_name || input.methodName || "").trim();
  const limit = Math.max(1, Math.min(Number(input.limit || 20), 80));
  const previewChars = Math.max(120, Math.min(Number(input.preview_chars || 800), 4000));
  const includeSourcePreview = input.include_source_preview !== false
    && input.includeSourcePreview !== false;
  ...
  const endpointHint = (fn) => {
    try {
      const source = Function.prototype.toString.call(fn);
      const match = source.match(/['"`]((?:https?:\\/\\/[^'"`\\s<>{}|\\\\]+|\\/[A-Za-z0-9_./~:%?&=#,+-]{2,}))[`'"]/);
      return match ? match[1] : null;
    } catch {
      return null;
    }
  };
  const payloadKeyHints = (fn) => {
    const skip = new Set([
      "apply",
      "bind",
      "call",
    ...
    const add = (value) => {
      ...
      if (/^(http|https|function|return|var|let|const)$/.test(text)) return;
      if (!values.includes(text)) values.push(text);
    };
    try {
      const source = Function.prototype.toString.call(fn);
      for (const match of source.matchAll(/(?:^|[^\\w$])['"]?([A-Za-z_$][\\w$]{1,48})['"]?\\s*:/g)) {
        add(match[1]);
        if (values.length >= 24) break;
      }
      if (values.length < 24) {
        for (const match of source.matchAll(/\\.([A-Za-z_$][\\w$]{1,48})\\b/g)) {
          add(match[1]);
          if (values.length >= 24) break;
        }
      }
      if (values.length < 24) {
        for (const match of source.matchAll(/\\[['"]([A-Za-z_$][\\w$]{1,48})['"]\\]/g)) {
          add(match[1]);
          if (values.length >= 24) break;
        }
      }
  ...
  const describeValue = (value, path) => {
    ...
    if (valueType === "function") {
      item.callable = true;
      item.function_name = value.name ? String(value.name) : null;
      item.arity = Number(value.length || 0);
      try {
        const source = Function.prototype.toString.call(value);
        item.source_chars = source.length;
        item.is_async = /^\\s*async\\b/.test(source);
        item.endpoint_hint = endpointHint(value);
        item.payload_key_candidates = payloadKeyHints(value);
        if (includeSourcePreview) item.source_preview = safeString(source, previewChars);
      } catch (error) {
        item.source_error = error && error.message ? String(error.message) : String(error);
      }
      return item;
    }
    if (value && (valueType === "object" || valueType === "function")) {
      try {
        ...
        const methods = allKeys
          .map((key, index) => {
            try {
              ...
              return {
                ...
                function_name: child.name ? String(child.name) : null,
                endpoint_hint: endpointHint(child),
                payload_key_candidates: payloadKeyHints(child),
          ...
          .filter(Boolean);
        item.method_count = methods.length;
        item.methods = methods
          .sort((left, right) => right.score - left.score || left.index - right.index)
          .slice(0, limit)
          .map(({ score, index, ...method }) => method);
      } catch (error) {
        item.keys_error = error && error.message ? String(error.message) : String(error);
      }
      return item;
    }
  ...
  if (!objectPath) {
    return { ok: false, reason: "object_path is required." };
  }
  const segments = parsePath(objectPath);
  let current = window;
  const traversed = [];
  ...
  const objectDescription = describeValue(current, objectPath);
  let methodDescription = null;
  if (methodName) {
    if (current == null || typeof current === "undefined") {
      methodDescription = { path: `${objectPath}.${methodName}`, exists: false };
    } else {
      try {
        methodDescription = describeValue(
          current[methodName],
          `${objectPath}.${methodName}`,
        );
      } catch (error) {
        methodDescription = {
          path: `${objectPath}.${methodName}`,
          exists: false,
...
""".strip()
_CLIENT_CALL_EXPRESSION = """
/*__crxzipple_browser_client_call__*/
async (raw) => {
  const input = raw && typeof raw === "object" ? raw : {};
  const objectPath = String(input.object_path || input.objectPath || "").trim();
  const methodName = String(input.method_name || input.methodName || "").trim();
  const maxResultChars = Math.max(1000, Math.min(Number(input.max_result_chars || input.maxResultChars || 60000), 200000));
  const resultStoreKey = String(
    input.result_store_key
      || input.resultStoreKey
      || `client_call_${Date.now()}_${Math.random().toString(16).slice(2)}`
  );
  const parsePath = (path) => String(path || "")
    .replace(/^window\\./, "")
    .split(".")
  ...
  const resolvePath = (path) => {
    ...
    return { ok: true, value: current, traversed };
  };
  const inputArguments = () => {
    if (Array.isArray(input.arguments)) return input.arguments;
    if (Array.isArray(input.args)) return input.args;
    if (Object.prototype.hasOwnProperty.call(input, "argument")) return [input.argument];
  ...
  const overview = (value) => {
    ...
    if (Array.isArray(value)) {
      ...
      item.sample = value.slice(0, 3).map((entry) => overview(entry));
      return item;
    }
    if (value && typeof value === "object") {
      const keys = Object.keys(value);
      item.key_count = keys.length;
      item.keys = keys.slice(0, 24);
      return item;
    }
    if (type === "string") item.preview = value.length > 160 ? `${value.slice(0, 157)}...` : value;
    else if (type !== "function" && type !== "undefined") item.value = value;
    return item;
  };
  const cloneForJson = (value, depth = 0, seen = new WeakSet()) => {
    ...
    seen.add(value);
    if (Array.isArray(value)) {
      const out = value.slice(0, 100).map((entry) => cloneForJson(entry, depth + 1, seen));
      if (value.length > 100) out.push({ __truncated_items__: value.length - 100 });
      return out;
    }
    if (typeof value === "object") {
      ...
      for (const key of keys.slice(0, 120)) {
        try {
          out[key] = cloneForJson(value[key], depth + 1, seen);
        } catch (error) {
          out[key] = `[Unreadable ${error && error.message ? String(error.message) : String(error)}]`;
        }
      }
      if (keys.length > 120) out.__truncated_keys__ = keys.length - 120;
      return out;
    }
    return String(value);
  };
  ...
  const cheapestCabin = (cabinInfoDescs) => {
    if (!Array.isArray(cabinInfoDescs)) return null;
    const candidates = [];
    for (const cabin of cabinInfoDescs) {
      const fares = cabin && Array.isArray(cabin.fareInfoDescList)
        ? cabin.fareInfoDescList
        : [];
      for (const fare of fares) {
        const total = asNumber(fare && fare.totalPrice);
        const price = asNumber(fare && (fare.lprice ?? fare.price));
        const tax = asNumber(fare && fare.taxPrice);
        if (total === null && price === null) continue;
        candidates.push({
          cabinLevelName: cabin.cabinLevelName ?? null,
          ccode: cabin.ccode ?? null,
          ctype: cabin.ctype ?? null,
          productCode: fare.productCode ?? null,
          priceSource: fare.priceSource ?? null,
          lprice: fare.lprice ?? fare.price ?? null,
          taxPrice: fare.taxPrice ?? null,
          totalPrice: fare.totalPrice ?? null,
          _sortTotal: total ?? ((price ?? 0) + (tax ?? 0)),
          _sortPrice: price ?? total,
        });
      }
    }
    candidates.sort((left, right) => {
      const leftTotal = left._sortTotal ?? Number.POSITIVE_INFINITY;
      const rightTotal = right._sortTotal ?? Number.POSITIVE_INFINITY;
      if (leftTotal !== rightTotal) return leftTotal - rightTotal;
      return (left._sortPrice ?? Number.POSITIVE_INFINITY)
        - (right._sortPrice ?? Number.POSITIVE_INFINITY);
    });
    const picked = candidates[0] || null;
    if (!picked) return null;
    const { _sortTotal, _sortPrice, ...publicPicked } = picked;
    return publicPicked;
  };
  const travelShoppingInsight = (value) => {
    const flightItems = value
      && value.data
      && Array.isArray(value.data.flightItems)
      ? value.data.flightItems
      : null;
    if (!flightItems) return null;
    const trips = [];
    for (let index = 0; index < flightItems.length; index += 1) {
      const item = flightItems[index] || {};
      const info = firstObject(
        Array.isArray(item.flightInfos) ? item.flightInfos[0] : null,
        item.flightInfo,
      );
      const sort = firstObject(info.flightSort, item.flightSort);
      const segments = Array.isArray(info.flightSegments)
        ? info.flightSegments
        : Array.isArray(item.flightSegments)
          ? item.flightSegments
          : [];
      const firstSegment = firstObject(segments[0]);
      const lastSegment = firstObject(segments[segments.length - 1], firstSegment);
      const cabin = cheapestCabin(
        Array.isArray(item.cabinInfoDescs)
          ? item.cabinInfoDescs
          : Array.isArray(info.cabinInfoDescs)
            ? info.cabinInfoDescs
            : [],
      );
      const sortPrice = sort.price ?? null;
      const sortPriceWithTax = sort.priceWithTax ?? null;
      const priceForSort = asNumber(
        cabin && cabin.totalPrice !== null && typeof cabin.totalPrice !== "undefined"
          ? cabin.totalPrice
          : sortPriceWithTax ?? sortPrice,
      );
      const standby = item.standby === true || info.standby === true || sort.standby === true;
      if (priceForSort === null || priceForSort <= 0 || standby) continue;
      trips.push({
        index,
        price_for_sort: priceForSort,
        route: {
          dep_code: firstSegment.orgCode ?? null,
          dep_name: firstSegment.orgName ?? null,
          dep_date: firstSegment.fltDate ?? null,
          dep_time: firstSegment.orgTime ?? null,
          arr_code: lastSegment.destCode ?? null,
          arr_name: lastSegment.destName ?? null,
          arr_date: lastSegment.arriDate ?? null,
          arr_time: lastSegment.destTime ?? null,
        },
        flights: segments.map((segment) => {
          const code = segment.carrierCode ?? segment.airlineCode ?? "";
          const number = segment.carrierNo ?? segment.flightNo ?? "";
          return `${code}${number}`.trim();
        }).filter(Boolean),
        segment_count: segments.length,
        segments: segments.slice(0, 4).map((segment) => ({
          flight: `${segment.carrierCode ?? segment.airlineCode ?? ""}${segment.carrierNo ?? segment.flightNo ?? ""}`.trim(),
          dep_code: segment.orgCode ?? null,
          dep_name: segment.orgName ?? null,
          dep_date: segment.fltDate ?? null,
          dep_time: segment.orgTime ?? null,
          arr_code: segment.destCode ?? null,
          arr_name: segment.destName ?? null,
          arr_date: segment.arriDate ?? null,
          arr_time: segment.destTime ?? null,
        })),
        duration_minutes: info.duration ?? firstSegment.fltSpanTime ?? null,
        sort_price: sortPrice,
        sort_price_with_tax: sortPriceWithTax,
        cheapest_cabin: cabin,
      });
    }
    trips.sort((left, right) => left.price_for_sort - right.price_for_sort);
    const uniqueTrips = [];
    ...
    return {
      ...
      result_message: value.resultMsg ?? null,
      currency_code: value.currencyCode ?? null,
      total_flight_items: flightItems.length,
      summarized_trip_count: trips.length,
      unique_trip_count: uniqueTrips.length,
      top_trips: uniqueTrips.slice(0, 5),
      top_trips_policy: "deduplicated_by_flight_route_time_price",
      numeric_unit_policy: "preserve_api_values",
    };
  };
  const insight = (value) => travelShoppingInsight(value);
  if (!objectPath) {
    return { ok: false, reason: "object_path is required." };
  }
  const resolved = resolvePath(objectPath);
  if (!resolved.ok) {
    return {
      ok: false,
      object_path: objectPath,
      ...
      missing_segment: resolved.missing_segment,
      traversed: resolved.traversed,
    };
  }
  let thisArg = window;
  let callable = resolved.value;
  if (methodName) {
    if (resolved.value == null || typeof resolved.value === "undefined") {
      return {
        ok: false,
        object_path: objectPath,
        method_name: methodName,
        reason: "object path resolved to empty value",
        traversed: resolved.traversed,
      };
    }
    thisArg = resolved.value;
    try {
      callable = resolved.value[methodName];
    } catch (error) {
      return {
        ok: false,
        object_path: objectPath,
  ...
  if (typeof callable !== "function") {
    return {
      ...
      target_type: valueType(callable),
      traversed: resolved.traversed,
    };
  }
  const args = inputArguments();
  try {
    const result = await callable.apply(thisArg, args);
    window.__crxzipple_client_call_results = window.__crxzipple_client_call_results || {};
    window.__crxzipple_client_call_results[resultStoreKey] = result;
    const cloned = cloneForJson(result);
    let resultJson = "";
    try {
      resultJson = JSON.stringify(cloned);
    } catch (error) {
      resultJson = "";
    }
    const out = {
      ...
      result_ref: resultStoreKey,
      result_type: valueType(result),
      result_overview: overview(result),
      result_json_chars: resultJson.length,
    };
    const resultInsight = insight(result);
    if (resultInsight) out.result_insight = resultInsight;
    if (!resultJson || resultJson.length <= maxResultChars) {
      out.result = cloned;
      out.result_truncated = false;
    } else {
      out.result_preview = resultJson.slice(0, Math.max(0, maxResultChars - 3)) + "...";
    ...
    return {
      ...
      stack_preview: error && error.stack ? String(error.stack).slice(0, 2000) : null,
    };
  }
}
""".strip()
    ...
    def execute(
        ...
        *,
        ...
        if kind == "runtime-probe-client":
            return _runtime_probe_client_action(page=page, payload=payload)
        if kind == "runtime-call-client":
            return _runtime_call_client_action(page=page, payload=payload)
...
def _runtime_probe_client_action(
    *,
    page: Any,
    payload: Mapping[str, Any],
) -> dict[str, Any]:
    object_path = _payload_text_any(payload, "object_path", "objectPath", "path")
    if object_path is None:
        raise BrowserValidationError("payload.object_path is required for runtime-probe-client.")
    method_name = _payload_text_any(payload, "method_name", "methodName")
    limit = min(_payload_int_any(payload, "limit", minimum=1) or 20, 80)
    preview_chars = min(
        _payload_int_any(
            payload,
            "preview_chars",
    ...
    include_source_preview = _payload_bool_any(
        payload,
    ...
    probe_payload = {
        ...
        "include_source_preview": (
            True if include_source_preview is None else include_source_preview
        ),
    }
    raw_result = _evaluate_runtime_probe_client(page, probe_payload)
    if not isinstance(raw_result, Mapping):
        raise BrowserValidationError("Browser runtime client probe returned an invalid result.")
    result = dict(_json_safe_payload(raw_result))
    if result.get("ok") is not True:
        reason = _payload_text_any(result, "reason") or "not available"
        return {
            ...
            **{
                ...
                if key not in {"kind", "ok", "object_path", "method_name", "reason"}
            },
        }
    return {
        "kind": "runtime-probe-client",
...
def _runtime_call_client_action(
    *,
    page: Any,
    payload: Mapping[str, Any],
) -> dict[str, Any]:
    object_path = _payload_text_any(payload, "object_path", "objectPath", "path")
    if object_path is None:
        raise BrowserValidationError("payload.object_path is required for runtime-call-client.")
    method_name = _payload_text_any(payload, "method_name", "methodName")
    arguments = _payload_value_any(payload, "arguments", "args")
    if arguments is not None and not isinstance(arguments, (list, tuple)):
        raise BrowserValidationError("payload.arguments must be a list.")
    call_arguments: list[Any]
    if arguments is not None:
        call_arguments = [_decode_jsonish_argument(item) for item in arguments]
    elif "argument" in payload:
        call_arguments = [_decode_jsonish_argument(payload.get("argument"))]
    elif "payload" in payload:
        call_arguments = [_decode_jsonish_argument(payload.get("payload"))]
    else:
        call_arguments = []
    ...
    result_store_key = _payload_text_any(payload, "result_store_key", "resultStoreKey")
    call_payload = {
        "object_path": object_path,
        "method_name": method_name,
        "arguments": _json_safe_payload(call_arguments),
        "max_result_chars": max_result_chars,
        "result_store_key": result_store_key,
    }
    raw_result = _evaluate_runtime_client_expression(
        page,
        expression=_CLIENT_CALL_EXPRESSION,
        payload=call_payload,
    )
    if not isinstance(raw_result, Mapping):
        raise BrowserValidationError("Browser runtime client call returned an invalid result.")
    result = dict(_json_safe_payload(raw_result))
    if result.get("ok") is not True:
        reason = _payload_text_any(result, "reason") or "not available"
        return {
            ...
            "object_path": object_path,
            "method_name": method_name,
            "argument_count": len(call_arguments),
            "arguments": _json_safe_payload(call_arguments),
            "reason": reason,
            **{
                key: value
    ...
    return {
        ...
        "max_result_chars": max_result_chars,
        "arguments": _json_safe_payload(call_arguments),
        "argument_key_candidates": _call_argument_key_candidates(call_arguments),
        **result,
    }


def _evaluate_runtime_probe_client(
    page: Any,
    payload: Mapping[str, Any],
) -> Any:
    return _evaluate_runtime_client_expression(
        page,
        expression=_CLIENT_PROBE_EXPRESSION,
        payload=payload,
    )


def _evaluate_runtime_client_expression(
    page: Any,
    *,
    expression: str,
    payload: Mapping[str, Any],
    ...
    try:
        ...
        if not _is_transient_runtime_probe_failure(exc):
            raise
    time.sleep(0.05)
    return page.evaluate(expression, payload)


def _decode_jsonish_argument(value: Any) -> Any:
...
def _call_argument_key_candidates(arguments: list[Any]) -> list[str]:
    values: list[str] = []
...
def _is_transient_runtime_probe_failure(exc: Exception) -> bool:
    message = " ".join(str(exc).split()).lower()
    return (
        "expecting value" in message
        and "line 1 column 1" in message
    )
