...
class WorkbenchReadModelProvider:
    ...
    def get_run_view(
        ...
        *,
        ...
        candidate_runs = _safe_list_runs_for_session(self.run_query, run.session_key)
        session_runs = _session_runs_for_run(
            self.run_query,
            run,
            candidate_runs=candidate_runs,
        )
        tool_runs = _safe_list_tool_runs_for_runs(
            self.tool_query,
            _tool_scope_run_ids(
                self.run_query,
                session_runs,
        ...
        tool_runs = _tool_runs_with_execution_fallback(
            self.run_query,
            self.tool_query,
            session_runs,
            tool_runs=tool_runs,
        ...
        if include_timeline:
            ...
            # The run view is refreshed frequently while a turn executes. Keep its
            # embedded timeline scoped to the selected run; historical turns remain
            # available through turn summaries and the steps endpoint.
            timeline_runs = (run,)
            timeline_llm_invocations = _llm_invocations_for_runs(
                self.run_query,
                self.llm_query,
                timeline_runs,
            )
        ...
        loop_health = _loop_health_for_workbench(
            self.run_query,
            run,
            llm_invocations=llm_invocations,
        )
    ...
    def list_step_views(
        ...
        *,
        ...
        selected_turn_id = _optional_text(turn_id)
        if selected_turn_id is not None:
            selected_runs = tuple(
                session_run
                for session_run in session_runs
                if session_run.id == selected_turn_id
                or _turn_id(session_run) == selected_turn_id
            )
        ...
        if run.status is OrchestrationRunStatus.COMPLETED:
            ...
            steps.append(
                _step(
                    ...
                    markdown=_failure_guidance_markdown(
...
def _safe_list_tool_runs_for_runs(
    tool_query: WorkbenchToolRunQueryPort | None,
    run_ids: tuple[str, ...],
) -> list[ToolRun] | None:
    if tool_query is None:
    ...
    if callable(scoped_list):
        try:
            ...
            return None
    return _safe_list_tool_runs(tool_query)


def _tool_runs_with_execution_fallback(
    run_query: OrchestrationRunQueryPort,
    tool_query: WorkbenchToolRunQueryPort | None,
    ...
    *,
    ...
    for run in runs:
        if not _run_may_have_execution_items(run):
            continue
        tool_run_ids.extend(_execution_tool_run_ids_for_run(run_query, run.id))
    if not tool_run_ids:
        return tool_runs
    runs_by_id: dict[str, ToolRun] = {}
    for tool_run_id in dict.fromkeys(tool_run_ids):
        try:
            tool_run = tool_query.get_tool_run(tool_run_id)
        except Exception:
            continue
        runs_by_id[tool_run.id] = tool_run
...
def _step_has_visible_narrative(step: TurnStepView) -> bool:
    if _optional_text(step.summary) is not None:
        return True
    if _optional_text(step.markdown) is not None:
        return True
    return False
...
def _tool_interaction_result_content(
    items: tuple[WorkbenchTimelineItem, ...],
) -> dict[str, Any]:
...
def _tool_interaction_lifecycle_content(
    content: dict[str, Any],
) -> dict[str, Any]:
    ...
    for key in (
        ...
        if isinstance(value, str):
            compact[key] = _truncate(value, limit=240)
...
def _timeline_item_from_tool_execution_item(
    ...
    *,
    ...
    request_render_snapshot_id = _request_render_snapshot_id(run, summary=summary)
...
def _timeline_content_for_tool_execution_item(
    ...
    *,
    ...
    if item.kind is ExecutionStepItemKind.TOOL_CALL:
        ...
        result_session_item_id = _summary_text(summary, "result_session_item_id")
        result_summary = (
            _summary_text(summary, "result_summary")
            or _summary_text(summary, "summary")
            or _summary_text(summary, "tool_result_summary")
        )
...
def _timeline_tool_execution_plan(
    ...
    for key in _TIMELINE_TOOL_EXECUTION_PLAN_KEYS:
        value = plan.get(key)
        if isinstance(value, str) and value.strip():
            sanitized[key] = _truncate(value, limit=240)
...
def _summary_tool_result_excerpt(summary: dict[str, object]) -> str | None:
    ...
    for label, key in (
        ...
        value = _summary_text(summary, key)
        if value is not None:
            lines.append(f"{label}: {_truncate(value, limit=240)}")
    read_handles = _summary_read_handles(summary)
    if read_handles:
        lines.append(
            "read_handles: "
...
def _timeline_items_from_llm_response_items(
    ...
    *,
    ...
    for item_index, response_item in enumerate(response_items):
        if _response_item_is_internal_control_tool(response_item):
            continue
...
def _response_item_is_internal_control_tool(response_item: Any) -> bool:
...
def _timeline_content_from_response_item(response_item: Any) -> dict[str, Any]:
    ...
    if kind == "reasoning" and not bool(getattr(response_item, "user_timeline_candidate", False)):
        return {
    ...
    payload = _timeline_visible_payload(
        dict(getattr(response_item, "content_payload", {}) or {}),
    )
    content: dict[str, Any] = {}
...
def _timeline_visible_payload(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        key: value
        for key, value in payload.items()
        if key not in _TIMELINE_DEBUG_PAYLOAD_KEYS
    }
    ...
    *,
    ...
    for bundle in bundles:
        ...
        if step.kind is ExecutionStepKind.LLM:
            tool_only_streak = (
                tool_only_streak + 1
                if _llm_bundle_is_tool_only(bundle, llm_query=llm_query)
                else 0
            )
            views.extend(
...
def _continuation_decision_step_views(
    ...
    *,
    ...
    for index, item in enumerate(bundle.items):
        ...
        provider_transport = _optional_text(provider_state.get("transport"))
        previous_response_id = _optional_text(provider_state.get("previous_response_id"))
        fallback_reason = _optional_text(provider_state.get("fallback_reason"))
        end_turn_label = (
        ...
        if provider_transport is not None:
            badges.append(StatusBadgeModel(label=provider_transport, tone="neutral"))
        if fallback_reason is not None:
            badges.append(StatusBadgeModel(label="Continuation fallback", tone="warning"))
        views.append(
            _step(
                run=run,
                turn_id=turn_id,
...
def _chain_llm_step_view(
    ...
    *,
    ...
    request_render_snapshot_id = (
        _summary_text_from_items(bundle.items, "request_render_snapshot_id")
        or _metadata_str(run, "request_render_snapshot_id")
    )
    step_error = step.error_payload
    summary = (
        step_error.message
        if step_error is not None and step.status is ExecutionStepStatus.FAILED
        else _llm_summary(run, llm_invocation=llm_invocation)
    )
    if diagnostics:
...
def _response_item_visible_text(payload: dict[str, object]) -> str | None:
    ...
    for item in summary:
        if isinstance(item, dict):
            item_text = _optional_text(item.get("text"))
        else:
...
def _request_render_snapshot_id(
    ...
    *,
    ...
    return (
        _summary_text(summary, "request_render_snapshot_id")
        or _metadata_str(run, "request_render_snapshot_id")
    )
...
def _trace_route(trace: TraceContext) -> str:
    ...
    focus_id = _trace_focus_id(trace)
    if focus_id:
        return f"{route}?focus_id={focus_id}"
    return route


def _trace_focus_id(trace: TraceContext) -> str | None:
    for value in (
...
def _tool_runs_for_run(
    ...
    tool_runs = _safe_list_tool_runs_for_runs(tool_query, (run_id,))
    if tool_runs is None:
        return ()
...
def _tool_scope_run_ids(
    ...
    *,
    ...
    scoped_ids: list[str] = []
    for run in runs:
        for source_run in _display_source_runs(
            run_query,
            run,
            candidate_runs=candidate_runs,
        ):
    ...
    if candidates is not None:
        for candidate in candidates:
            ...
            if not isinstance(followup_payload, dict):
                continue
            requester_run_id = _optional_text(followup_payload.get("requester_run_id"))
...
def _llm_invocation_for_run(
    ...
    if llm_query is None or not _run_may_have_execution_items(run):
        return None
...
def _run_may_have_execution_items(run: OrchestrationRun) -> bool:
        ...
        memory=(
            WorkbenchKeyValueSection(
                ...
                items=(
                    ...
                    _kv("Runtime request mode", _metadata_str(run, "runtime_request_mode") or "-"),
                ),
            ),
        ),
        agent=(
...
def _loop_health_for_workbench(
    run_query: OrchestrationRunQueryPort,
    run: OrchestrationRun,
    *,
    llm_invocations: tuple[Any, ...],
...
def _response_item_resolver_from_invocations(
    ...
    for invocation in llm_invocations:
        for response_item in tuple(getattr(invocation, "response_items", ()) or ()):
            item_id = _optional_text(getattr(response_item, "id", None))
...
def _loop_health_section(
    loop_health: dict[str, object] | None,
) -> WorkbenchKeyValueSection:
    if loop_health is None:
        return WorkbenchKeyValueSection(
            id="loop_health",
            title="Loop Health",
            items=(_kv("Status", "unavailable", tone="neutral"),),
        )
    warnings = [
        warning
    ...
    max_streak = _optional_int(loop_health.get("max_tool_only_streak")) or 0
    current_streak = _optional_int(loop_health.get("current_tool_only_streak")) or 0
    ...
    return WorkbenchKeyValueSection(
        ...
        title="Loop Health",
        items=(
            _kv("Warnings", warning_text, tone=warning_tone),
            _kv("Max tool-only streak", str(max_streak)),
            _kv("Current tool-only streak", str(current_streak)),
            _kv("Tool-only segments", str(segment_count)),
            _kv(
                "Validation delta",
                validation_delta_text,
...
def _failure_guidance_markdown(
    *,
    message: str,
    code: str | None,
    ...
    guidance = _failure_guidance_items(
...
def _failure_guidance_items(
    *,
    ...
    if code == "access_not_ready":
        ...
        setup_kind = (
            _optional_text(setup_flow.get("setup_flow", {}).get("kind"))
        ...
        resource_type = _optional_text(details.get("resource_type"))
        resource_id = _optional_text(details.get("resource_id"))
        ...
        for requirement in _access_requirement_labels(details.get("access")):
...
def _access_requirement_labels(value: object) -> tuple[str, ...]:
    if not isinstance(value, dict):
        return ()
    labels: list[str] = []
    direct_requirement = _optional_text(value.get("requirement"))
