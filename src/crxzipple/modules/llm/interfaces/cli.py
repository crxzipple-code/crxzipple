...
from crxzipple.modules.llm.interfaces.cli_payloads import (
    invocation_request_preview_report,
    load_json,
    parse_messages,
    parse_tool_schemas,
    profile_input_from_cli_args,
)
...
def build_cli() -> typer.Typer:
    ...
    def invoke_llm(
        ...
        response_format_payload = load_json(response_format, "--response-format")
        overrides_payload = load_json(overrides, "--overrides")
        invocation = container.require(AppKey.LLM_SERVICE).invoke(
            InvokeLlmInput(
                llm_id=llm_id,
                messages=parse_messages(messages),
                tool_schemas=parse_tool_schemas(tool_schemas),
                response_format=(
