...
class OpenAICodexResponsesRenderer:
    ...
    def preview_input(
        ...
        if requested_provider_transport_input(render_input) == "websocket":
            endpoint = _websocket_endpoint_from_http(rendered.endpoint)
...
def _websocket_endpoint_from_http(http_endpoint: str) -> str:
    return http_endpoint.replace("https://", "wss://", 1).replace("http://", "ws://", 1)
