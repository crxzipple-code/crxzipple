...
def collect_access_inventory_for_operations(
    query: Any,
    *,
    include_ready: bool,
) -> dict[str, object]:
    source = access_inventory_input_from_settings(
        query.settings_query_service,
        environment=query.settings_environment,
    )
    return collect_access_inventory_from_read_models(
        source,
        check_readiness=operations_access_readiness_checker(
...
def access_inventory_input_from_settings(
    ...
    *,
    ...
    return AccessInventoryInput(
        assets=tuple(access_asset_read_model(record) for record in provider.list_assets()),
        credential_bindings=tuple(
            credential_binding_read_model(record)
            for record in provider.list_credential_bindings()
        ),
        consumer_bindings=tuple(
            access_consumer_binding_read_model(record)
            for record in provider.list_consumer_bindings()
        ),
    )


def access_asset_read_model(record: Any) -> AccessAssetDetailReadModel:
...
def credential_binding_read_model(record: Any) -> CredentialBindingReadModel:
    return CredentialBindingReadModel(
        ...
        source_ref=credential_source_ref_for_operations(record),
...
def credential_source_ref_for_operations(record: Any) -> str:
...
def access_consumer_binding_read_model(
    record: Any,
) -> AccessConsumerBindingReadModel:
...
def operations_access_readiness_checker(
    access_service: Any,
):
    def check(
