...
import {
  loadTraceContextSnapshotById,
  loadTraceContextSnapshot,
  loadTraceData,
  loadTraceInvocationRuntimeRequestPreview,
  loadTraceLinkedEntityDetail,
  loadTraceRuntimeRequestPreview,
...
const contextSnapshot = ref<TraceContextSnapshot | null>(null);
...
const activeFocusId = computed(() => {
  const raw = route.query.focus_id;
...
const completedStatusCount = computed(() => (
  (statusTotals.value.success ?? 0) + (statusTotals.value.completed ?? 0)
));
const partialStatusCount = computed(() => (
  (statusTotals.value.accepted ?? 0)
  + (statusTotals.value.queued ?? 0)
  + (statusTotals.value.waiting ?? 0)
));
const totalTraceEventCount = computed(() => traceSummary.value?.event_count ?? traceEvents.value.length);
const visibleTraceEventCount = computed(() => activeEvents.value.length);
const keyTraceEventCount = computed(() => activeEvents.value.filter((event) => event.key_event).length);
...
const displaySummaryStatus = computed(() => traceSummary.value?.status);
const displaySummaryDuration = computed(() => traceSummary.value?.duration_ms);
const selectedEventPayloadFieldCount = computed(() => {
  const payload = selectedEvent.value?.payload;
  return payload ? Object.keys(payload).length : 0;
});
const selectedEventRelatedEventCount = computed(() => {
  const event = selectedEvent.value;
  if (!event) return 0;
  const stepId = event.trace.step_id;
  if (stepId) {
    return activeEvents.value.filter((item) => item.trace.step_id === stepId).length;
  }
  return 1;
});
const selectedEventLinkedEntityCount = computed(() => selectedEvent.value?.linked_entities.length ?? 0);
const selectedEventFailureRows = computed(() => {
  const event = selectedEvent.value;
  if (!event || event.status !== "failed") return [];
  const payload = event.payload ?? {};
  return [
    { label: t("trace.detail.errorCode"), value: payloadText(payload, "error_code", "code", "status_code") },
    { label: t("trace.detail.errorMessage"), value: payloadText(payload, "error_message", "message", "reason") || event.summary },
    { label: t("trace.detail.provider"), value: payloadText(payload, "provider", "provider_id", "provider_name") },
    { label: t("trace.detail.model"), value: payloadText(payload, "model", "model_name", "llm_model") },
    { label: t("trace.detail.requestId"), value: payloadText(payload, "request_id", "provider_request_id", "trace_request_id") },
...
const contextSnapshotRows = computed(() => {
  ...
  const renderedTokens = (
    metadataOptionalNumber(metadata.debug_body_estimated_tokens)
    ?? contextEstimateTokenTotal(snapshot.estimate)
  );
  return [
    ...
    { label: t("workbench.context.archivedRefs"), value: formatNumber(metadataNumber(metadata.archived_ref_count)) },
    { label: t("workbench.context.debugBodyTokens"), value: formatNumber(renderedTokens) },
    { label: t("workbench.context.providerInputTokens"), value: formatOptionalNumber(metadata.estimated_provider_input_tokens) },
    { label: t("workbench.context.directTranscriptTokens"), value: formatOptionalNumber(metadata.draft_input_estimated_tokens) },
...
const contextSnapshotRiskRows = computed<Array<{
  ...
  const metadata = contextSnapshot.value?.metadata;
  if (!metadata) return [];
...
const contextSnapshotDiagnosticRows = computed(() => {
  ...
  if (!snapshot) return [];
  const metadata = snapshot.metadata;
  const runtimeInput = snapshotRuntimeLlmRequestDraftMetadata(snapshot);
  const requestMetadata = snapshotLlmRequestMetadata(snapshot);
  return [
    {
      label: t("table.invocationId"),
      value: shortId(textValue(contextRuntimeRequestPreview.value?.provider_request_options?.invocation_id, ""), 24),
    },
    {
      label: t("workbench.context.requestMetadataSnapshot"),
      value: shortId(textValue(requestMetadata.request_render_snapshot_id, snapshot.id), 24),
    },
    {
...
const contextSnapshotSessionNodeRows = computed(() => {
  const refs = contextSnapshot.value?.metadata.session_message_node_refs;
...
watch(
  () => [route.params.traceId, route.query.focus_id],
  async ([traceIdParam, focusIdParam]) => {
    loadingTrace.value = true;
    loadError.value = null;
    try {
      const traceId = typeof traceIdParam === "string" ? traceIdParam : null;
      const focusId = typeof focusIdParam === "string" ? focusIdParam : null;
      const loaded = await loadTraceData(traceId, focusId);
...
watch(
  ...
  ([nextRunId]) => {
    void refreshTraceContextSnapshot(
...
async function refreshTraceContextSnapshot(nextRunId: string | null): Promise<void> {
  ...
  let preview = invocationId
    ? await loadTraceInvocationRuntimeRequestPreview(invocationId, nextRunId)
    : await loadTraceRuntimeRequestPreview(nextRunId);
  if (invocationId && !preview) {
    preview = await loadTraceRuntimeRequestPreview(nextRunId);
  }
  const snapshotId = typeof preview?.request_render_snapshot_id === "string"
    ? preview.request_render_snapshot_id.trim()
    : "";
  const snapshot = snapshotId
    ? await loadTraceContextSnapshotById(snapshotId)
    : await loadTraceContextSnapshot(nextRunId);
...
async function loadTraceContextDebugBody(): Promise<void> {
  const snapshot = contextSnapshot.value;
  ...
  try {
    const loaded = snapshot?.id
      ? await loadTraceContextSnapshotById(snapshot.id, { includeDebugBody: true })
      : typeof currentRunId === "string" && currentRunId.trim()
        ? await loadTraceContextSnapshot(currentRunId, { includeDebugBody: true })
        : null;
    if (loaded) {
      contextSnapshot.value = loaded;
    }
  } catch (error) {
    contextDebugBodyError.value = error instanceof Error ? error.message : String(error);
  } finally {
    contextDebugBodyLoading.value = false;
  }
}
...
function snapshotRuntimeLlmRequestDraftMetadata(snapshot: TraceContextSnapshot): Record<string, unknown> {
  const value = snapshot.provider_attachments.runtime_request_draft;
  return isRecord(value) ? value : {};
}

function snapshotRuntimeContractMetadata(snapshot: TraceContextSnapshot): Record<string, unknown> {
  const value = snapshot.metadata.runtime_contract;
  return isRecord(value) ? value : {};
}

function snapshotLlmRequestMetadata(snapshot: TraceContextSnapshot): Record<string, unknown> {
  ...
  return {
    ...
    debug_body_estimated_tokens: snapshot.metadata.debug_body_estimated_tokens,
    draft_input_estimated_tokens: snapshot.metadata.draft_input_estimated_tokens,
    estimated_provider_input_tokens: snapshot.metadata.estimated_provider_input_tokens,
...
function contextRouteMetadata(): Record<string, unknown> {
  ...
  return {
    ...requestMetadata,
    ...(snapshot?.metadata ?? preview?.request_render_snapshot_metadata ?? {}),
  };
}
...
function linkedEntitySupportsDetail(entity: TraceLinkedEntity): boolean {
  return (
    ...
    || entity.type === "llm_invocation"
    || entity.type === "llm_invocation_id"
...
function linkedEntityDetailPayloadJson(detail: WorkbenchLinkedEntityDetail | null): string {
  if (!detail) return "";
  return JSON.stringify(detail.payload, null, 2);
}

function linkedEntityReplayInputRows(detail: WorkbenchLinkedEntityDetail | null): Array<{ label: string; value: string }> {
...
function linkedEntityRuntimeHintRows(detail: WorkbenchLinkedEntityDetail | null): Array<{ label: string; value: string }> {
...
function linkedEntityProviderInputRows(detail: WorkbenchLinkedEntityDetail | null): Array<{ label: string; value: string }> {
  const renderReport = isRecord(detail?.payload.provider_render_report)
    ? detail.payload.provider_render_report
    : {};
  const wirePreview = isRecord(detail?.payload.provider_wire_preview)
    ? detail.payload.provider_wire_preview
    : {};
  if (!Object.keys(renderReport).length && !Object.keys(wirePreview).length) return [];
  const toolSurface = isRecord(renderReport.tool_surface)
    ? renderReport.tool_surface
    : {};
  const rows = [
    {
      label: t("workbench.entityDetail.providerRenderer"),
      value: textValue(wirePreview.renderer_id ?? renderReport.renderer_id),
    },
    {
      label: t("workbench.entityDetail.providerTransport"),
      value: textValue(wirePreview.transport ?? renderReport.transport),
    },
    {
      label: t("workbench.entityDetail.providerStrategy"),
      value: textValue(wirePreview.render_strategy ?? renderReport.render_strategy),
    },
    {
      label: t("workbench.entityDetail.providerEndpoint"),
      value: textValue(wirePreview.endpoint),
    },
    {
      label: t("workbench.entityDetail.providerTools"),
      value: formatOptionalNumber(toolSurface.provider_visible_tool_count ?? wirePreview.tool_count),
    },
    {
      label: t("workbench.entityDetail.providerDroppedTools"),
      value: formatOptionalNumber(toolSurface.dropped_tool_schema_count),
    },
    {
      label: t("workbench.entityDetail.providerToolMapping"),
      value: providerToolMappingValue(toolSurface.provider_tool_mapping),
    },
    {
      label: t("workbench.entityDetail.providerContinuation"),
      value: providerContinuationValue(wirePreview),
    },
  ];
  return rows.filter((row) => row.value && row.value !== "-");
}

function providerToolMappingValue(value: unknown): string {
  ...
  for (const item of value) {
    ...
    if (textValue(item.trace_status, "") === "runtime_tool_surface") {
      traced += 1;
    } else {
      untraced += 1;
    }
    if (samples.length >= 3) continue;
    const providerName = textValue(item.provider_name, "");
    const target = textValue(item.node_id ?? item.source_id ?? item.tool_id, "");
    if (providerName && target) samples.push(`${providerName}->${target}`);
  }
  const parts = [`traced=${traced}`, `untraced=${untraced}`];
  if (samples.length) parts.push(`sample=${samples.join(", ")}`);
  return parts.join("; ");
}

function providerContinuationValue(wirePreview: Record<string, unknown>): string {
  const previousResponseId = textValue(wirePreview.previous_response_id, "");
  const hasPrevious = wirePreview.has_previous_response_id === true || Boolean(previousResponseId);
  const deltaMode = wirePreview.input_delta_mode === true;
  const deltaCount = formatOptionalNumber(wirePreview.input_delta_count);
  const baselineCount = formatOptionalNumber(wirePreview.input_baseline_count);
  if (hasPrevious && deltaMode) {
    return `previous_response_id=${previousResponseId || "true"}; delta=${deltaCount}; baseline=${baselineCount}`;
  }
  if (hasPrevious) return `previous_response_id=${previousResponseId || "true"}`;
  if (deltaMode) return `delta=${deltaCount}; baseline=${baselineCount}`;
  return "-";
}
...
function timelineEntities(event: TraceEventView): TraceLinkedEntity[] {
  ...
  const entities: TraceLinkedEntity[] = [];
  addTraceEntity(entities, "run_id", trace.run_id);
  addTraceEntity(entities, "turn_id", trace.turn_id);
  addTraceEntity(entities, "step_id", trace.step_id);
  addTraceEntity(entities, "llm_invocation_id", trace.llm_invocation_id);
  addTraceEntity(entities, "tool_run_id", trace.tool_run_id);
  addTraceEntity(entities, "session_item_id", trace.session_item_id);
  addTraceEntity(entities, "request_render_snapshot_id", trace.request_render_snapshot_id);
  addTraceEntity(entities, "artifact_id", trace.artifact_id);
  addTraceEntity(entities, "event_id", trace.source_event_id ?? event.event_id);
  return uniqueTraceEntities(entities);
}

function eventDetailValue(key: string, event: TraceEventView): string {
  if (key === "source") return event.trace.source_event_name ?? event.trace.source_event_id ?? "-";
  if (key === "observation") return event.trace.observed_event_name ?? event.trace.observed_event_id ?? "-";
  if (key === "caused") {
    return event.trace.tool_run_id
      ?? event.trace.llm_invocation_id
      ?? event.trace.run_id
      ?? "-";
  }
  return "-";
...
function addTraceEntity(items: TraceLinkedEntity[], type: string, id: string | null | undefined): void {
  if (!id) return;
  items.push({ type, id });
}

function uniqueTraceEntities(items: TraceLinkedEntity[]): TraceLinkedEntity[] {
    ...
    <aside class="trace-filters scroll-area">
      ...
      <section class="filter-section">
        ...
        <div class="time-range-grid">
          <input :value="formatTraceDateTime(traceSummary?.started_at)" readonly />
          <input :value="formatTraceDateTime(traceSummary?.completed_at)" readonly />
        </div>
      </section>
      ...
      <section class="filter-section">
        ...
        <label>
          ...
          <strong>{{ statusTotals.cancelled ?? 0 }}</strong>
        </label>
        <label>
      ...
      <div class="filter-footer">
        <span>{{ t("trace.filterFound", { count: keyTraceEventCount, filtered: `${visibleTraceEventCount} / ${totalTraceEventCount}` }) }}</span>
    ...
    <section class="trace-main scroll-area">
      ...
      <UiCard v-if="activeTraceView === 'timeline'" class="trace-summary trace-summary--timeline">
        ...
        <div>
          ...
          <strong class="mono">{{ shortId(activeFocusId) }}</strong>
        </div>
        <div>
        ...
        <div>
          ...
          <strong>{{ visibleTraceEventCount }} / {{ totalTraceEventCount }}</strong>
        </div>
        <div>
    ...
    <aside class="event-inspector scroll-area">
      ...
      <UiCard v-if="selectedEvent" class="event-card">
        ...
        <div class="inspector-tabs">
          ...
          <button type="button">{{ t("trace.tabs.payload") }} ({{ selectedEventPayloadFieldCount }})</button>
          <button type="button">{{ t("trace.tabs.logs") }} ({{ selectedEventPayloadFieldCount }})</button>
          <button type="button">{{ t("trace.tabs.events") }} ({{ selectedEventRelatedEventCount }})</button>
          <button type="button">{{ t("trace.tabs.linked") }} ({{ selectedEventLinkedEntityCount }})</button>
        </div>
        ...
        <dl v-if="selectedEventFailureRows.length" class="event-details event-details--summary">
          <div v-for="row in selectedEventFailureRows" :key="row.label">
            <dt>{{ row.label }}</dt>
            <dd>{{ row.value }}</dd>
          </div>
        </dl>
        ...
        <RouterLink
          ...
          :key="entity.type"
          class="entity-link"
          :to="`/workbench/traces/${selectedEvent.trace.trace_id}`"
        >
          <span>{{ entity.type }}</span>
          <code>{{ entity.id }}</code>
        ...
        <div v-if="selectedEntityDetail || entityDetailError" class="entity-detail-card">
          <template v-if="selectedEntityDetail">
            ...
            <dl
              v-if="linkedEntityProviderInputRows(selectedEntityDetail).length"
              class="entity-detail-surface"
            >
              <div class="entity-detail-section-title">
                <dt>{{ t("workbench.entityDetail.providerInput") }}</dt>
                <dd>{{ t("workbench.entityDetail.providerInputHint") }}</dd>
              </div>
              <div
                v-for="row in linkedEntityProviderInputRows(selectedEntityDetail)"
                :key="row.label"
              >
                <dt>{{ row.label }}</dt>
                <dd :title="row.value">{{ row.value }}</dd>
        ...
        <div class="quick-actions">
          ...
          <RouterLink :to="`/workbench/traces/${selectedEvent.trace.trace_id}`">
            <ExternalLink :size="16" />
      ...
      <UiCard v-if="contextSnapshot || contextRuntimeRequestPreview || loadingContextSnapshot" class="event-card trace-context-snapshot-card">
        <div class="trace-context-snapshot-head">
          <div>
            <h2>{{ t("trace.contextSnapshot.title") }}</h2>
            <p>{{ t("trace.contextSnapshot.subtitle") }}</p>
          </div>
          <UiBadge :tone="contextSnapshot || contextRuntimeRequestPreview ? 'success' : 'info'">
            {{ contextSnapshot || contextRuntimeRequestPreview ? t("workbench.context.historyDelivery") : t("common.loading") }}
          </UiBadge>
        </div>

        <div v-if="loadingContextSnapshot && !contextSnapshot" class="trace-context-snapshot-loading">
          {{ t("trace.contextSnapshot.loading") }}
        </div>

        <template v-if="contextSnapshot || contextRuntimeRequestPreview">
          ...
          <div class="context-request-panel">
            <template v-if="contextActualRequestTab === 'xml'">
              ...
              />
              <div v-else class="trace-context-snapshot-loading context-request-empty context-debug-body-loader">
                ...
                <UiButton
                  ...
                  @click="loadTraceContextDebugBody"
                >
                  {{ contextDebugBodyLoading ? t("common.loading") : t("workbench.context.loadDebugBody") }}
                </UiButton>
